...
class TransientAbsorption(qtw.QMainWindow, Ui_MainWindow):
    ...
    def ta_dynamics(self, one_shot=bool):
        if one_shot == True:
            ...
            self.graphicsView.plot(self.ta_array[0], self.ta_array[1], pen=(0, 114, 189), symbolPen = 'w', symbol='o',
                                   symbolSize=3, clear = True)
        elif one_shot == False:
    ...
    def save(self):
        ...
        np.savetxt(file_spec, ta_data, fmt='%1.2f', header=self.delay_string[1:-1], comments='#sec ')          
